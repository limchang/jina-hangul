...
def get_category(place_id):
    try:
        content = fetch_jina(f"https://m.place.naver.com/restaurant/{place_id}/home")
        # "식당명\n\n카테고리" 패턴
        m = re.search(r'# [^\n]+\n+([가-힣,·\s]{2,20}(?:요리|전문|식당|집|점|탕|국|밥|찜|구이|면|수산|분식|오리|고기|한식|중식|일식|포차|횟집|낙지|회|장|갈비|만두|순대|보쌈|족발|라멘|추어탕|해장국|해물))', content[:2000])
        if m:
